...
abstract class AnalysisWorkDao {
    @Query("""SELECT i.id AS itemId, i.file_path AS imagePath, j.status,
        NULL AS ocrText, NULL AS payloadJson FROM items i
        LEFT JOIN analysis_jobs j ON j.item_id = i.id ORDER BY i.created_at DESC, i.id""")
    abstract fun observeItems(): Flow<List<DetailEntry>>

    @Query("""SELECT j.id AS jobId, j.item_id AS itemId, i.file_path AS filePath,
    ...
    @Query("""SELECT i.id AS itemId, i.file_path AS imagePath, j.status,
        r.ocr_text AS ocrText, r.payload_json AS payloadJson FROM items i
    ...
    @Query("UPDATE analysis_jobs SET status = 'running', stage = 'ocr', started_at = :now WHERE id = :id AND status = 'queued'")
    ...
    @Query("UPDATE analysis_jobs SET status = 'succeeded', error_message = NULL, finished_at = :now WHERE id = :id AND status = 'running'")
    ...
    @Query("""UPDATE analysis_jobs SET
    ...
    @Query("UPDATE analysis_jobs SET status = 'cancelled', finished_at = :now WHERE id = :id AND status IN ('queued', 'running')")
    abstract suspend fun cancel(id: String, now: Long)

    @Query("UPDATE analysis_jobs SET status = 'cancelled', finished_at = :now WHERE item_id = :itemId AND status IN ('queued', 'running')")
